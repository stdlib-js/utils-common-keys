...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof commonKeys, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an array of common property names (two objects)', function test( t ) {
	...
	actual = commonKeys( obj1, obj2 );
	expected = [ 'a', 'b', 'c' ];
	t.deepEqual( actual, expected, 'returns expected keys' );

	actual = commonKeys( obj1, obj3 );
	expected = [ 'c', 'd' ];
	t.deepEqual( actual, expected, 'returns expected keys' );

	actual = commonKeys( obj2, obj3 );
	expected = [ 'c' ];
	t.deepEqual( actual, expected, 'returns expected keys' );
...
tape( 'the function returns an array of common property names (multiple objects)', function test( t ) {
	...
	actual = commonKeys( obj1, obj2, obj3, obj4 );
	expected = [ 'a' ];
	t.deepEqual( actual, expected, 'returns expected keys' );

	actual = commonKeys( obj2, obj3, obj4 );
	expected = [ 'a' ];
	t.deepEqual( actual, expected, 'returns expected keys' );

	actual = commonKeys( obj3, obj2, obj1 );
	expected = [ 'a', 'c', 'd' ];
	t.deepEqual( actual, expected, 'returns expected keys' );

	actual = commonKeys( obj4, obj2, obj1 );
	expected = [ 'a', 'b' ];
	t.deepEqual( actual, expected, 'returns expected keys' );

	actual = commonKeys( obj2, obj1, obj3 );
	expected = [ 'a', 'c', 'd' ];
	t.deepEqual( actual, expected, 'returns expected keys' );
